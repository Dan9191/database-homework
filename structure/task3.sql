...
SELECT
    ...
    d.DepartmentName,
    r.RoleName,
    (SELECT STRING_AGG(p.ProjectName, ', ') FROM Projects p WHERE p.DepartmentID = emp.DepartmentID) AS ProjectNames,
    (SELECT STRING_AGG(t.TaskName, ', ') FROM Tasks t WHERE t.AssignedTo = emp.EmployeeID) AS TaskNames,
    (SELECT COUNT(*) - 1 FROM ManagerHierarchy mh_count WHERE mh_count.RootManagerID = emp.EmployeeID) AS TotalSubordinates
FROM
